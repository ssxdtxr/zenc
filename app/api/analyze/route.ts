...
Задачи:
1. Определи подтемы темы. Каждая подтема — это отдельный механизм или паттерн, а не аспект одного и того же. Правило: если два пункта отвечают на вопрос "что это / зачем / как использовать" про один и тот же объект — это ОДНА подтема, а не несколько. Не создавай отдельные подтемы для "концепция X", "паттерны X", "применение X" — это всё одна подтема "X".
