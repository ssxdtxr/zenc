...
Формат ответа — СТРОГО JSON без markdown:
{
  ...
  "mainIdea": "главная идея в 2-4 предложениях — что это такое, зачем нужно и как работает/применяется",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const cacheKey = trimmedRecommendation
      ? `v4:user:${await getOrCreateUserId()}:${topicName}:${subtopicName}:${level}:${createHash("sha256").update(trimmedRecommendation).digest("hex").slice(0, 16)}`
      : `v4:shared:${topicName}:${subtopicName}:${level}`
