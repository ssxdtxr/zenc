...
Формат ответа — СТРОГО JSON без markdown:
{
  ...
  "sections": [
    { "heading": "название смысловой части", "explanation": "полноценное объяснение этой части — 3-6 содержательных абзацев (не сжимай в пару предложений)" }
  ],
  "watchOut": "на что обратить внимание — развёрнутый абзац о нюансах, подводных камнях или распространённых заблуждениях, с объяснением почему это заблуждение возникает",
  "definitions": [
...
Правила:
- sections: разбей объяснение на смысловые части. Если подтема узкая (один механизм/концепция) — ОДНА секция, но глубокая (не превращай узость темы в повод объяснить поверхностно — раскрой её полностью: определение, как устроено внутри, почему именно так, когда применяется, что рядом). Если подтема широкая и объединяет несколько механизмов, паттернов или этапов — 3-8 секций, по одной на каждый смысловой блок, с своим заголовком. Не дроби искусственно узкую подтему и не сливай в одну секцию то, что заслуживает разделения. Целься в объяснение уровня главы учебника, а не краткой шпаргалки — читатель должен закрыть страницу, реально поняв механизм, а не просто пробежав по верхам.
...
- explanation и watchOut: разделяй абзацы двойным переносом строки (\n\n) внутри строки — это обычный многострочный текст, не массив
