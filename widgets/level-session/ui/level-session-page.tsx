  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", background: BG, overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 960, margin: "0 auto", padding: "22px 28px 80px" }}>
        ...
        {/* NAV */}
        <nav style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", borderRadius: 20, background: "rgba(var(--fg-rgb),0.06)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", border: "1px solid rgba(var(--fg-rgb),0.12)", boxShadow: "0 10px 40px rgba(var(--shadow-rgb),0.45)", marginBottom: 22 }}>
          <button onClick={() => router.back()} style={{ width: 40, height: 40, flexShrink: 0, borderRadius: 12, border: "1px solid rgba(var(--fg-rgb),0.14)", background: "rgba(var(--fg-rgb),0.04)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ChevronLeftIcon size={20} color="rgba(var(--fg-rgb),0.85)" />
          </button>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "rgba(var(--fg-rgb),0.42)" }}>{topic?.name} · {subtopicName}</div>
        ...
        {/* DONE / RESULT */}
        {phase === "done" && current && (
          <div style={{ borderRadius: 24, background: "rgba(var(--fg-rgb),0.06)", backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", border: "1px solid rgba(var(--fg-rgb),0.12)", boxShadow: "0 18px 60px rgba(var(--shadow-rgb),0.5)", padding: "28px 30px" }}>
            {saving ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, padding: "40px 0" }}>
                ...
                <p style={{ margin: 0, fontSize: 14, color: "rgba(var(--fg-rgb),0.5)" }}>Сохраняю результат…</p>
              ...
              <motion.div variants={staggerContainer(0.08)} initial="hidden" animate="show" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <motion.div variants={fadeInUp} style={{ padding: "26px 24px", borderRadius: 20, background: current.isComplete ? "linear-gradient(135deg,rgba(94,224,138,0.15),rgba(43,217,227,0.06))" : "linear-gradient(135deg,rgba(255,126,146,0.12),rgba(255,80,80,0.04))", border: `1px solid ${current.isComplete ? "rgba(94,224,138,0.3)" : "rgba(255,126,146,0.3)"}`, textAlign: "center" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 15, lineHeight: 1.6, color: "rgba(var(--fg-rgb),0.72)" }}>{current.finishSummary}</p>
                </motion.div>
                ...
                <motion.div variants={staggerContainer(0.05)} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  {[
                    ...
                    <motion.div key={s.label} variants={fadeInUp} style={{ padding: "14px", borderRadius: 14, background: "rgba(var(--fg-rgb),0.05)", border: "1px solid rgba(var(--fg-rgb),0.09)", textAlign: "center" }}>
                      <div className="font-display" style={{ fontSize: 20, fontWeight: 800, color: "var(--text)" }}>{s.value}</div>
                      <div style={{ fontSize: 11.5, color: "rgba(var(--fg-rgb),0.45)", fontWeight: 600, marginTop: 3 }}>{s.label}</div>
                    </motion.div>
                  ))}
                </motion.div>
                ...
                {savedCfg && (
                  <motion.div variants={fadeInUp} style={{ padding: "14px 18px", borderRadius: 14, background: savedCfg.bg, border: `1px solid ${savedCfg.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(var(--fg-rgb),0.5)" }}>НОВЫЙ СТАТУС ПОДТЕМЫ</span>
                ...
                <motion.div variants={fadeInUp} style={{ display: "flex", gap: 10 }}>
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.back()}
                    style={{ flex: 1, padding: "14px", borderRadius: 14, border: "1px solid rgba(var(--fg-rgb),0.16)", cursor: "pointer", background: "rgba(var(--fg-rgb),0.05)", color: "rgba(var(--fg-rgb),0.82)", fontWeight: 600, fontSize: 15, fontFamily: "inherit" }}>
                    ← К теории
                  </motion.button>
                </motion.div>
              </motion.div>
        ...
        {/* SESSION / FEEDBACK */}
        {phase !== "done" && (
          <div style={{ borderRadius: 24, background: "rgba(var(--fg-rgb),0.06)", backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", border: "1px solid rgba(var(--fg-rgb),0.12)", boxShadow: "0 18px 60px rgba(var(--shadow-rgb),0.5)", padding: "26px 30px 30px" }}>

            {/* Progress */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: lvl.color, boxShadow: `0 0 8px ${lvl.color}` }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: "rgba(var(--fg-rgb),0.55)" }}>Вопрос {questionCount + 1} · ИИ решает когда хватит</span>
              </div>
            ...
            {phase === "feedback" && current && (
              <motion.div key="feedback" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
                {lastQuestion && (
                  <div style={{ padding: "13px 16px", borderRadius: 14, background: "rgba(var(--fg-rgb),0.04)", border: "1px solid rgba(var(--fg-rgb),0.08)", marginBottom: 10 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "rgba(var(--fg-rgb),0.35)", marginBottom: 5 }}>ВОПРОС</div>
                    <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "rgba(var(--fg-rgb),0.72)" }}>{lastQuestion}</p>
                  </div>
                )}
                {lastAnswer && (
                  <div style={{ padding: "13px 16px", borderRadius: 14, background: "rgba(var(--fg-rgb),0.04)", border: "1px solid rgba(var(--fg-rgb),0.08)", marginBottom: 12 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "rgba(var(--fg-rgb),0.35)", marginBottom: 5 }}>МОЙ ОТВЕТ</div>
                    <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "rgba(var(--fg-rgb),0.72)", whiteSpace: "pre-wrap" }}>{lastAnswer}</p>
                  </div>
                )}
                  ...
                  return (
                    <div>
                      <div style={{ padding: "16px 18px", borderRadius: 14, background: v.bg, border: `1.5px solid ${v.border}`, marginBottom: 10 }}>
                        ...
                        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: "rgba(var(--fg-rgb),0.82)" }}>{current.evaluation}</p>
                      </div>
                      ...
                      {isWrong && current.correctAnswer && (
                        <div style={{ padding: "14px 18px", borderRadius: 14, background: "rgba(94,224,138,0.08)", border: "1px solid rgba(94,224,138,0.28)", marginBottom: 10 }}>
                          ...
                          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.62, color: "rgba(var(--fg-rgb),0.88)" }}>{current.correctAnswer}</p>
                        </div>
                      )}
                      ...
                      {current.explanation && (
                        <div style={{ padding: "14px 18px", borderRadius: 14, background: "linear-gradient(135deg,rgba(155,107,255,0.1),rgba(43,217,227,0.05))", border: "1px solid rgba(155,107,255,0.22)", marginBottom: 14 }}>
                          ...
                          <RichText text={current.explanation} className="[&_p]:!text-[rgba(var(--fg-rgb),0.78)] [&_p]:!text-sm" />
                        </div>
                      )}
            ...
            {phase === "session" && current?.question && !loading && (
              <motion.div key="session" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
                <p style={{ margin: "0 0 20px", fontSize: 18, lineHeight: 1.65, color: "rgba(var(--fg-rgb),0.92)" }}>{current.question}</p>
                ...
                {current.questionType === "choice" && current.options ? (
                  <motion.div variants={staggerContainer(0.05)} initial="hidden" animate="show" style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
                    {current.options.map((opt, i) => {
                      ...
                      return (
                        <motion.button key={i} variants={fadeInUp} whileHover={{ x: 2 }} whileTap={{ scale: 0.985 }} onClick={() => selectOption(opt)} style={{ textAlign: "left", padding: "14px 18px", borderRadius: 14, border: `1.5px solid ${sel ? lvl.border : "rgba(var(--fg-rgb),0.1)"}`, background: sel ? lvl.bg : "rgba(var(--fg-rgb),0.04)", color: sel ? "var(--text)" : "rgba(var(--fg-rgb),0.75)", fontWeight: sel ? 600 : 500, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, fontFamily: "inherit" }}>
                          <span style={{ width: 24, height: 24, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, background: sel ? lvl.color : "rgba(var(--fg-rgb),0.08)", color: sel ? "#08070f" : "rgba(var(--fg-rgb),0.4)", border: `1px solid ${sel ? "transparent" : "rgba(var(--fg-rgb),0.12)"}` }}>{String.fromCharCode(65 + i)}</span>
                          {opt}
                        </motion.button>
                      )
                    })}
                  ...
                  <textarea
                    ...
                    rows={4}
                    style={{ width: "100%", resize: "vertical", padding: "14px 16px", borderRadius: 14, background: "rgba(var(--fg-rgb),0.04)", border: `1.5px solid ${answer ? lvl.border : "rgba(var(--fg-rgb),0.12)"}`, outline: "none", color: "var(--text)", fontSize: 15, lineHeight: 1.6, fontFamily: "inherit", boxSizing: "border-box", marginBottom: 14 }}
                  />
                )}

                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                  <span style={{ fontSize: 12.5, color: "rgba(var(--fg-rgb),0.35)" }}>⌘ Enter — отправить</span>
