  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", background: BG, overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1040, margin: "0 auto", padding: "22px 28px 80px" }}>
        ...
        {/* NAV */}
        <nav style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", borderRadius: 20, background: "rgba(var(--fg-rgb),0.06)", backdropFilter: "blur(24px) saturate(150%)", WebkitBackdropFilter: "blur(24px) saturate(150%)", border: "1px solid rgba(var(--fg-rgb),0.12)", boxShadow: "0 10px 40px rgba(var(--shadow-rgb),0.45), inset 0 1px 0 rgba(var(--fg-rgb),0.18)" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <h1 className="font-display" style={{ fontWeight: 700, fontSize: 19, margin: "1px 0 0", color: "var(--text)" }}>Практика: {lvl.label}</h1>
          </div>
          {exercises.length > 0 && (
            <span style={{ fontSize: 13, fontWeight: 600, color: "rgba(var(--fg-rgb),0.5)", flexShrink: 0 }}>{doneCount}/{exercises.length}</span>
          )}
        ...
        {loadingExercises && (
          <div style={{ paddingTop: 80, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: "rgba(var(--fg-rgb),0.5)", fontWeight: 500 }}>Придумываю задания…</p>
        ...
        {allDone && (
          <motion.div initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 28 }} style={{ marginTop: 20, padding: "24px 26px", borderRadius: 22, background: avgScore >= 80 ? "linear-gradient(135deg,rgba(94,224,138,0.15),rgba(43,217,227,0.06))" : avgScore >= 50 ? "linear-gradient(135deg,rgba(255,187,92,0.15),rgba(255,150,50,0.05))" : "linear-gradient(135deg,rgba(255,126,146,0.12),rgba(255,80,80,0.04))", border: `1px solid ${avgScore >= 80 ? "rgba(94,224,138,0.3)" : avgScore >= 50 ? "rgba(255,187,92,0.3)" : "rgba(255,126,146,0.3)"}`, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "rgba(var(--fg-rgb),0.4)", marginBottom: 8 }}>ИТОГ ПРАКТИКИ · {lvl.label.toUpperCase()}</div>
            ...
            <p style={{ margin: "8px 0 0", fontSize: 14.5, color: "rgba(var(--fg-rgb),0.65)" }}>
        ...
        {!loadingExercises && exercises.length > 0 && (
          <motion.div variants={staggerContainer(0.08)} initial="hidden" animate="show" style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 16 }}>
            {exercises.map((ex, i) => {
              ...
              return (
                <motion.div key={i} variants={fadeInUp} style={{ borderRadius: 22, background: "rgba(var(--fg-rgb),0.06)", backdropFilter: "blur(26px)", WebkitBackdropFilter: "blur(26px)", border: "1px solid rgba(var(--fg-rgb),0.12)", boxShadow: "0 12px 40px rgba(var(--shadow-rgb),0.4)", overflow: "hidden" }}>

                  {/* Exercise header */}
                  <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid rgba(var(--fg-rgb),0.08)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <span style={{ width: 24, height: 24, borderRadius: 8, background: "rgba(var(--fg-rgb),0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12, color: "rgba(var(--fg-rgb),0.5)", flexShrink: 0 }}>{i + 1}</span>
                    ...
                    <p style={{ margin: 0, fontSize: 14.5, lineHeight: 1.6, color: "rgba(var(--fg-rgb),0.65)" }}>{ex.description}</p>
                  </div>
                    ...
                    {!s?.result ? (
                      <>
                        <textarea
                          ...
                          rows={6}
                          style={{ width: "100%", resize: "vertical", padding: "14px 16px", borderRadius: 14, background: "rgba(var(--fg-rgb),0.04)", border: `1.5px solid ${s?.answer ? lvl.border : "rgba(var(--fg-rgb),0.1)"}`, outline: "none", color: "var(--text)", fontSize: 13.5, lineHeight: 1.65, fontFamily: "var(--font-mono, monospace)", boxSizing: "border-box" }}
                        />
                      ...
                      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {/* User's answer recap */}
                        <div style={{ padding: "12px 14px", borderRadius: 12, background: "rgba(var(--fg-rgb),0.04)", border: "1px solid rgba(var(--fg-rgb),0.08)" }}>
                          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", color: "rgba(var(--fg-rgb),0.35)", marginBottom: 6 }}>МОЁ РЕШЕНИЕ</div>
                          <pre style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: "rgba(var(--fg-rgb),0.7)", whiteSpace: "pre-wrap", fontFamily: "var(--font-mono, monospace)" }}>{s.answer}</pre>
                        </div>
                        ...
                        {v && (
                          <div style={{ padding: "14px 16px", borderRadius: 14, background: v.bg, border: `1.5px solid ${v.border}` }}>
                            ...
                            <p style={{ margin: 0, fontSize: 14, lineHeight: 1.62, color: "rgba(var(--fg-rgb),0.82)" }}>{s.result.feedback}</p>
                          </div>
                        )}
                        ...
                        {s.result.betterApproach && (
                          <div style={{ padding: "14px 16px", borderRadius: 14, background: "linear-gradient(135deg,rgba(155,107,255,0.1),rgba(43,217,227,0.05))", border: "1px solid rgba(155,107,255,0.22)" }}>
                            ...
                            <p style={{ margin: 0, fontSize: 14, lineHeight: 1.62, color: "rgba(var(--fg-rgb),0.78)" }}>{s.result.betterApproach}</p>
                          </div>
                        )}
                        ...
                        <motion.button
                          ...
                          style={{ alignSelf: "flex-start", padding: "9px 16px", borderRadius: 10, border: "1px solid rgba(var(--fg-rgb),0.14)", background: "rgba(var(--fg-rgb),0.04)", color: "rgba(var(--fg-rgb),0.6)", fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}
                        >
                          Попробовать снова
                        </motion.button>
                      </motion.div>
        ...
        {!loadingExercises && (
          <div style={{ marginTop: 24, display: "flex", gap: 12, flexWrap: "wrap" }}>
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.back()} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 20px", borderRadius: 14, border: "1px solid rgba(var(--fg-rgb),0.16)", cursor: "pointer", background: "rgba(var(--fg-rgb),0.04)", color: "rgba(var(--fg-rgb),0.8)", fontWeight: 600, fontSize: 15, fontFamily: "inherit" }}>
              ← Вернуться
            </motion.button>
