...
const LEVEL_CONFIG: Record<string, { label: string; desc: string; bg: string; border: string }> = {
  basic:        { label: "Базовый",     desc: "Определения и концепции", bg: "var(--surface-2)",    border: "var(--border)" },
  intermediate: { label: "Средний",     desc: "Применение и нюансы",     bg: "var(--surface)",       border: "var(--border)" },
  advanced:     { label: "Продвинутый", desc: "Edge cases и детали",     bg: "var(--surface-hover)", border: "var(--border-strong)" },
}
    ...
    <AppShell>
      <div style={{ position: "relative", flex: 1, background: "var(--bg)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "22px 24px 80px" }}>
          ...
          {/* HEADER */}
          <div style={{ display: "flex", alignItems: "center", gap: 12, paddingBottom: 18, borderBottom: "1px solid var(--border)", flexWrap: "wrap" }}>
            ...
            {exercises.length > 0 && (
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-2)", flexShrink: 0 }}>{doneCount}/{exercises.length}</span>
            )}
          ...
          {loadingExercises && (
            <div style={{ paddingTop: 80, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
              ...
              <p style={{ margin: 0, fontSize: 14, color: "var(--text-2)", fontWeight: 500 }}>Придумываю задания…</p>
          ...
          {allDone && (() => {
            ...
            return (
              <motion.div initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 28 }} style={{ marginTop: 20, padding: "24px 26px", borderRadius: 18, background: c.bg, border: `1px solid ${c.border}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "var(--text-3)", marginBottom: 8 }}>ИТОГ ПРАКТИКИ · {lvl.label.toUpperCase()}</div>
                  ...
                  <motion.div key={i} variants={fadeInUp} style={{ borderRadius: 18, background: "var(--surface)", border: "1px solid var(--border)", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid var(--border)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        ...
                        <span style={{ padding: "3px 10px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: lvl.bg, color: "var(--text-2)", border: `1px solid ${lvl.border}` }}>{lvl.label}</span>
          ...
          {!loadingExercises && (
            <div style={{ marginTop: 24, display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/topic/${topicId}/subtopic/${encodeURIComponent(subtopicName)}/level/${level}`)} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 20px", borderRadius: 14, border: "none", cursor: "pointer", background: "var(--accent)", color: "#fff", fontWeight: 600, fontSize: 15, fontFamily: "inherit" }}>
                Тест этого уровня →
              </motion.button>
            </div>
          )}
